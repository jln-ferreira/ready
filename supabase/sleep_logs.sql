-- Kids profiles (not auth users) — any household member can manage
create table if not exists kids (
  id                   uuid primary key default gen_random_uuid(),
  household_id         uuid not null references households(id) on delete cascade,
  name                 text not null,
  date_of_birth        date,
  daily_sleep_goal_min int  not null default 660, -- 11h default
...
-- Sleep sessions linked to kids (not auth users)
