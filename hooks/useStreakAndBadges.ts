...
function computeBadges(
  ...
  // Month winner: most chore points in any month vs at least one other member
  ...
  const monthWinner = Object.values(monthMap).some(members => {
    if (Object.keys(members).length < 2) return false
...
export function useStreakAndBadges(
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [myChoresRes, myWaterRes, allChoresRes, allWaterRes, myActivityRes, allActivityRes] = await Promise.all([
        supabase.from('chore_logs')
          .select('done_by, done_date, points_earned')
          .eq('done_by', userId)
          .gte('done_date', since),
      ...
      const myLogs     = (myChoresRes.data ?? []) as ChoreLog[]
      ...
      const uniqueMembers = members.filter(m => {
        ...
        seen.add(m.user_id)
        // Exclude family admin accounts — they manage the household but don't compete
      ...
      if (!cancelled) {
        setData({ streak: current, longestStreak: longest, badges, householdStreaks })
        setLoading(false)
      }
    }
